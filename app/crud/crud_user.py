...
def create_user(db: Session, username: str, name: str, role: str, password: str, phone: str = None) -> User:
    ...
    u = User(username=username, name=name.strip(), role=role, phone=phone,
              pw=hash_pw(password), active=True)
    db.add(u)
    db.commit()
    db.refresh(u)
    return u
...
def update_user(db: Session, user_id: int, name: str = None, role: str = None, phone: str = None,
                 active: bool = None, password: str = None) -> User:
