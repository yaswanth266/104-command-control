...
from app.crud.crud_category import _require_active_ticket_type
...
def create_reason(db: Session, code: str, category_code: str, label: str, ticket_type: str = None) -> Reason:
    ...
    r = Reason(code=code, category_code=category_code, label=label.strip(), is_active=True, ticket_type=ticket_type)
    db.add(r)
    db.commit()
    db.refresh(r)
    return r

def update_reason(db: Session, code: str, label: str = None, is_active: bool = None, ticket_type: str = None) -> Reason:
    ...
    if ticket_type is not None:
        r.ticket_type = _require_active_ticket_type(db, ticket_type)
